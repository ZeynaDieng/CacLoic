<template>
  <section v-if="!loading" class="product-catalog-section mt-5">
    <div class="grid-container">
      <nav class="breadcrumb is-small mb-2 show-for-large" aria-label="breadcrumbs">
        <ul>
          <li>
          ...
          <li v-for="categogyLevel of categoryLevels" :key="categogyLevel._id">
            <NuxtLink :to="`/produits/categorie/${categogyLevel.slug}`">{{ categogyLevel.name }}</NuxtLink>
          </li>
          <li class="is-active">
            <NuxtLink
          ...
          <div class="small-12 medium-3 cell">
            <div class="search-filter">
              ...
              <div class="filter-box mt-2 mb-2" v-if="brandFilters.length">
                ...
                <b-field v-for="({brand}, index) of elementToDisplay(brandFilters, 'brand_filter')" :key="brand._id">
                  <b-checkbox
                    @input="runFilters"
                    v-model="brandFilters[index].selected"
                  >
                    <span class="is-capitalized">{{ brand.name }} ({{ brand.count }})</span>
                  </b-checkbox>
                </b-field>
                <div v-show="brandFilters.length > 4">
                  <b-button
                    type="is-ghost"
                    :icon-left="!showElements['brand_filter']? 'plus': 'minus'"
                    @click="$set(showElements, 'brand_filter', !showElements['brand_filter'])"
                    :label="!showElements['brand_filter']?`Plus (${ (brandFilters.length - 4) })`: `Moins`"
                  />
                </div>
              </div>
              ...
              <div class="filter-box mt-2 mb-2">
                ...
                <div class="divider mb-3"></div>
                <b-field>
                  <b-slider @change="runFilters" size="is-medium"
                            v-model="priceRange"
                            :min="minPrice || 0"
              ...
              <div class="filter-box mt-2 mb-2" v-if="categoryFilters.length">
                ...
                <b-field
                  v-for="({category}, index) of elementToDisplay(categoryFilters, 'category_filter')"
                  :key="category._id">
                  <b-checkbox
                    @input="runFilters"
                    v-model="categoryFilters[index].selected"
                  >
                    <span class="is-capitalized">{{ category.name }} ({{ category.count }})</span>
                  </b-checkbox>
                </b-field>
                <div v-show="categoryFilters.length > 4">
                  <b-button
                    type="is-ghost"
                    :icon-left="!showElements['category_filter']? 'plus': 'minus'"
                    @click="$set(showElements, 'category_filter', !showElements['category_filter'])"
                    :label="!showElements['category_filter']?`Plus (${ (categoryFilters.length - 4) })`: `Moins`"
                  />
                </div>
              </div>
              <!--END  filter components -->
              ...
              <div class="filter-box mt-2 mb-2" v-if="tagFilters.length">
                ...
                <div class="divider mb-3"></div>
                <b-collapse></b-collapse>
                <b-field v-for="({tag}, index) of elementToDisplay(tagFilters, 'tag_filter')" :key="tag._id">
                  <b-checkbox
                    @input="runFilters"
                ...
                <div v-show="tagFilters.length > 4">
                  <b-button
                    ...
                    @click="$set(showElements, 'tag_filter', !showElements['tag_filter'])"
                    :label="!showElements['tag_filter']?`Plus (${ (tagFilters.length - 4) })`: `Moins`"
                  />
                </div>
              </div>
              <!--END  filter components -->
              <!-- filter components -->
              <div class="filter-box mt-2 mb-2" v-for="(attribute, indexAtt) of attributeFilters" :key="indexAtt">
                <p class="mb-2 has-text-weight-medium">{{ attribute.name }}</p>
                <div class="divider mb-3"></div>
                <b-collapse></b-collapse>
                <b-field v-for="(term, indexTerm) of attribute.terms" :key="indexTerm">
                  <b-checkbox
                    @input="runFilters"
                    v-model="attributeFilters[indexAtt].terms[indexTerm].selected"
                  >{{ term.value.value }}
                  </b-checkbox>
                </b-field>
              </div>
              ...
              <div class="small-12 cell">
                <NuxtLink to="/produits/categorie/electromenager">
                  ...
                  <div class="category-banner"
                       v-if="bannerImage && bannerImage !== '' && bannerImage !== 'https://apicac.intech.sn/storage/undefined.webp'"
                       :style='`background-image: url("${bannerImage}");`'>

                  </div>
                  <div v-else class="category-banner"
                       style='background-image: url("https://apicac.intech.sn/storage/15-11-2022/cat_banner_bqvihy3-_8hc54zdza0bx.webp");'>

                  </div>
                </NuxtLink>
              </div>
              <div class="small-12 cell">
                <p class="pt-3 pb-3 pl-5 has-text-grey	">{{ pagination ? pagination.totalDocs : 0 }} produits trouvés</p>

                <div class="divider"></div>
              </div>
            </div>
            <ul class="grid-x bdordered-cell grid-margin-x grid-margin-y">
         <li v-for="product in computedProduct"
                  v-bind:key="Math.random()+product._id"
                  class="small-6 large-3 medium-4 cell">
                <product-card @addToCart="addProductToCart({product: product, quantityToBeAdd: 1})"
                              :product="product"/>
              </li>
              <div class="small-12 cell">
                <infinite-scroll
                ...
                >
                  <template v-if="isLoading">
                    ...
                    <span class="sr-only" v-if="isLoading">Chargement des produits</span>
                  </template>
                </infinite-scroll>
              </div>
...
export default {
  ...
  head() {
    return {
      title: "CAC SENEGAL | " + this.categoryName,
      meta: [
        {
          name: "viewport",
  ...
  async asyncData({$axios, route}) {
    try {
      ...
      const type = route.params.type === 'categorie' ? 'category' : route.params.type === 'marque' ? 'brand' : route.params.type === 'rechercher' ? 'search' : route.params.type === 'all' ? '' : 'tag';
  ...
  try {
    ...
    const { data, status } = await this.$axios.get(`/product?${this.$formatUrlFromQuery(query, params)}&page=${this.pagination.nextPage}`);
  ...
  if (!this.pagination.hasNextPage ) {
    ...
    formatCurrency(input) {
      return new Intl.NumberFormat('fr-FR', {style: 'currency', currency: 'XOF'}).format(input)
    },
    runFilters() {
    ...
    brandFilter() {
      const selectedBrands = this.brandFilters.filter(brandFilter => brandFilter.selected === true);
    ...
    tagFilter(filteredProduct) {
      const selectedTags = this.tagFilters.filter(tagFilter => tagFilter.selected === true)
    ...
    attributesFilter(filteredProduct) {
      ...
      const selectedAttributes = this.attributeFilters.map(attributeFilter => {
        const filteredTerms = attributeFilter.terms.filter(term => term.selected)
