  ...
  <div class="home-container ">
    ...
    <section class="banner-section">
      <div class="grid grid-cols-4 gap-4 container   ">
        ...
        <div
          class=" h-full bg-white pt-2 py-2 border-l-2 border-cac-500   overflow-y-scroll overflow-x-hidden hidden lg:block menu-container ">
            ...
            <li v-for="(category, index) of getAppMenu" :key="index" class="mx-4 left-menu-element ">
              <dl>
                <dt class="category-name">
                  <nuxt-link :to="`/produits/categorie/${category.slug}`"
                    class="flex flex-row items-center py-1 px-4  rounded-md text-gray-800 hover:text-cac-800 hover:bg-cac-200">
                ...
                <dd v-if="category.children.length > 0" class="dropDown-menu_wrapper " style="display: block;">
                  <ul class="dropDown-menu dropDown-menu grid grid-cols-4 rounded-md" data-v-d2447884="">
                    <li v-for="(item, index) in category.children" :key="index" class="" data-v-d2447884="">
                      <NuxtLink :to="`/produits/categorie/${item.slug}`" class=" nav-sub-menu-title" data-v-d2447884="">
                        {{ item.name }}</NuxtLink>
                      <NuxtLink v-for="(subItem, index) in item.children" :key="index"
                        :to="`/produits/categorie/${subItem.slug}`" class="dropDown-menu-link" data-v-d2447884="">
                        {{ subItem.name }}
                      </NuxtLink>
                    </li>
                  </ul>
    ...
    <div class="container home-sections">
      <HomeExpoB :hideProductsTitle="true" :limit=6 :sectionTitle="results.superPromos[0].title" :products="results.superPromos[0].products"
        :to="`/${results.superPromos[0].category.slug}`" class="hidden md:block " />
      <HomeExpoB :hideProductsTitle="true" :limit=2 :sectionTitle="results.superPromos[0].title" :products="results.superPromos[0].products"
        :to="`/${results.superPromos[0].category.slug}`" class=" md:hidden mx-2" />
      ...
      <!--    <section class="md:grid grid-cols-2 gap-4 hidden ">
            ...
            <br> -->
      <section class="mt-5 double-row-banner">
        <div v-if="results.smallBanners.length>0" class=" grid grid-cols-1 md:grid-cols-2 gap-3">
          <div v-for="(smallBanner, index) in results.smallBanners" :key="index"  class="long-banner">
            <nuxt-link to=""> <img class="img-responsive" :src="smallBanner.image_url" alt="" /></nuxt-link>
          </div>
      <!--     <div class="long-banner">
            <nuxt-link to=""> <img class="img-responsive" src="@/assets/images/banners-right.webp" alt="" /></nuxt-link>
          </div> -->
        </div>
      </section>
      ...
      <template v-for="(catSection, index)  of results.categorySection">
        <header
          class="section-heading md:mt-7 md:mb-4  heading-line is-size-4 is-size-5-mobile    justify-center  hidden md:flex  "
          :key="index">
        ...
        <section class="grid grid-cols-1 md:grid-cols-4  gap-4" :key="index">
          ...
          <template v-for="box in catSection.boxes">
            <nuxt-link :to="`/produits/categorie/${box.category.slug}`" v-if="box.main === true" :key="box._id"
              class=" bg-white rounded-md px-4 py-4  row-span-2 text-black ">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div v-for="(product, index) in  box.products" :key="index" class="h-32 rounded-md home-cat-img-wrapper "
                  :style="`background-image: url(${getStorageUrl() + 'storage/' + product.images[0]['filename']});`">
            ...
            <nuxt-link :to="`/produits/categorie/${box.category.slug}`" v-else :key="box._id"
              class="  bg-white rounded-md px-4 py-4  h-52 text-black ">
              <!-- TITLE -->
              ...
              <div class="grid grid-cols-2 gap-4">
                <div v-for="product in limitProductInCategorie(box)" :key="product._id"
                  class="h-32 rounded-md home-cat-img-wrapper "
                  :style="`background-image: url(${getStorageUrl() + 'storage/' + product.images[0]['filename']});`">
                  <picture class="img-bg">
                    <source type="image/webp" :srcset="`${getStorageUrl() + 'storage/' + product.images[0]['filename']}`">
                  </picture>
      ...
      <header
        class="section-heading md:mt-7 md:mb-4  heading-line is-size-4 is-size-5-mobile   hidden md:flex justify-center   ">
      ...
      <section class="mx-2">
        <ul
          class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-1 md:gap-3 sgrid-x bordbered-cell sgrid-margin-x sgrid-margin-y">
          ...
          <div class="xsmall-12 xcell" >
            <infinite-scroll :enough="enough" class="flex justify-center w-full h-40" @load-more="loadMore">
              <template v-if="isLoading">
                <svg class="w-8 h-8 mr-3 border-0 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="green" stroke-width="4"></circle>
                  <path class="opacity-75" fill="green"
                    d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                  </path>
                </svg>
                <span class="sr-only bg-cac-500" v-if="isLoading"> chargement en cours...</span>
              </template>
            </infinite-scroll>
          </div>
        </ul>
...
export default {
  ...
  async asyncData({ $axios, store }) {
    ...
    try {
      ...
      const [sections, superPromos, banners, smallBanners, categorySection] = await Promise.all([
        await $axios.get("/product?category=electromenager"),
        await $axios.get("/category-section/product"),
        // await $axios.get(`/product?page=${Math.random() * (20 - 3) + 3}`),
        // await $axios.get("/product_category/menu"),
        await $axios.get("/banner?type=BIG"),
        await $axios.get("/banner?type=SMALL"),
      ...
      results.superPromos = superPromos.data.data;
      results.banners = banners.data.data;
      results.smallBanners = smallBanners.data.data;
...
console.log(this.enough);
        ...
        if(this.results.pagination.page <= 5 ){
          ...
          if (status === 200) {
            if (this.results.pagination.page <= 5) {
              ...
              console.log(this.results.pagination.nextPage);
              this.isLoading = false;
              this.enough = true;
              this.$nuxt.$loading.finish()
  ...
  try {
    if (this.isLoading || !this.results.pagination.hasNextPage) {
      return;
    }
    
    if (this.results.pagination.page > 6) {
      this.isLoading = false;
      this.enough = true;
      this.$nuxt.$loading.finish();
      return;
    ...
    const { data, status } = await this.$axios.get(`/product?page=${this.results.pagination.nextPage}`);
