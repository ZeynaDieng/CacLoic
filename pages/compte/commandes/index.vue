...
<!-- @details-open="(row) => $buefy.toast.open(`Commande ${row.reference}`)" -->
        ...
        <nuxt-link :to="`commandes/${props.row._id}`">{{ props.row.reference }}</nuxt-link>
          </b-table-column>
          ...
          <b-table-column field="user.status" label="Status" sortable v-slot="props">
          ...
          <template #detail="props">
           ...
           <b-table-column field="items" label="Montant" sortable v-slot="props">
            <template v-if="showDetailIcon">
                      {{ props.row.transaction.total }} FCFA
            </template>
            <template v-else>
              <a @click="props.toggleDetails(props.row)">
                {{ props.row.user.firstName }} {{ props.row.user.lastName }}
...
export default {
  ...
  methods: {
    async getCustomerOrders() {
      ...
      const resp = await this.$repositories.order.all();
